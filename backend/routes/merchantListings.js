...
import {
  ...
  merchantListingListQuerySchema,
...
router.get("/list", validateRequest({ query: merchantListingListQuerySchema }), (req, res) => {
  ...
  db.get(
    ...
    (countErr, countRow) => {
      if (countErr) {
        logger.error("Merchant listings count DB error: " + countErr.message);
        return res.status(500).json({
          ok: false,
          error: "Database error while counting merchant listings"
        });
      }
...
router.post("/moderate/:id", validateRequest({ params: merchantListingIdParamsSchema, body: merchantListingModerateSchema }), requireAdminSecret, (req, res) => {
  const { id } = req.params;
...
router.get("/moderation-history/:id", validateRequest({ params: merchantListingIdParamsSchema }), requireAdminSecret, (req, res) => {
  const { id } = req.params;
